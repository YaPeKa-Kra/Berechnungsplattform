...
class CalcEngine:
    ...
    def calculate_steering_radius(self, half_wheelbase, steering_angle_deg):
        """
    ...
    def calculate_axle_to_center(self, steering_radius, steering_angle_deg):
        """
    ...
    def calculate_wall_to_center(self, axle_to_center, axle_to_wall_distance):
        """
        Calculates wall to center using the formula: Axle to center - axle to wall.
        """
        return axle_to_center - axle_to_wall_distance

    def calculate_alpha(self, wall_to_center, steering_radius):
        """
    ...
    def calculate_beta(self, steering_angle_deg, alpha_deg):
        """
    ...
    def calculate_arc_length(self, steering_radius, beta_deg):
        """
    ...
    def calculate_time_to_collision(self, arc_length, speed):
        """
        ...
        for current_axle_to_wall_dist in self.parameters.axle_to_wall_distances:
            for current_frame_to_wall_dist in self.parameters.frame_to_wall_distances:
                for i, case_params in enumerate(self.parameters.steering_speed_cases):
                    ...
                    s_radius_new = self.calculate_steering_radius(half_wheelbase_val, current_steering_angle_deg)
                    axle_center_new = self.calculate_axle_to_center(s_radius_new, current_steering_angle_deg)
                    wall_center_new = self.calculate_wall_to_center(axle_center_new, current_axle_to_wall_dist)
                    alpha_new_deg = self.calculate_alpha(wall_center_new, s_radius_new)
                    beta_new_deg = self.calculate_beta(current_steering_angle_deg, alpha_new_deg)
                    arc_len_new = self.calculate_arc_length(s_radius_new, beta_new_deg)
